...
public enum TabStripDividers {
    ...
    public static func visible(count: Int, selected: Int?, hovered: Int?, dragged: Int?) -> [Bool] {
        guard count > 1 else { return [] }
        let occupied = Set([selected, hovered, dragged].compactMap { $0 })
    ...
    public static func visible<Item: Equatable>(
        in order: [Item], selected: Item?, hovered: Item?, dragged: Item?
    ) -> [Bool] {
